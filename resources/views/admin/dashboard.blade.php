            ...
            <form id="filterForm" method="GET" class="d-flex align-items-center gap-2">
                <div class="position-relative">
                    <select name="bulan" class="form-select auto-submit shadow-sm border-secondary-subtle py-md-2"
                        ...
                        <option value="{{ $k }}" {{ $bulan == $k ? 'selected' : '' }}>{{ $v }}
                        </option>
                        @endforeach
                    </select>
                </div>
                ...
                <a href="{{ route('dashboard') }}"
                    class="btn btn-light border border-secondary-subtle shadow-sm py-md-2 px-md-3" title="Reset Filter">
            ...
            <div class="card-body">
                ...
                <div class="row">
                    <div class="col-6">
                        ...
                        <h5 class="fw-bold text-white">Rp{{ number_format($data['hari_ini']['masuk'], 0, ',', '.') }}
                        </h5>
                    </div>
                    <div class="col-6">
                        <small class="opacity-75">Pengeluaran</small>
                        <h5 class="fw-bold text-white">Rp{{ number_format($data['hari_ini']['keluar'], 0, ',', '.') }}
                        </h5>
                    </div>
                </div>
            </div>
...
<div class="row g-4">
    <div class="col-12 col-lg-8">
        <div class="card border-0 shadow-sm p-3">
            <h6 class="fw-bold mb-3 small"><i class="fas fa-chart-line me-2"></i>Growth Kas Tahunan
                ({{ $tahun }})</h6>
...
<script>
    ...
    Highcharts.chart('chartGrowth', {
        ...
        series: [{
            name: 'Pemasukan',
            color: '#1cc88a',
            data: @json($data['chart_masuk'] ?? [], JSON_NUMERIC_CHECK)
        }, {
            name: 'Pengeluaran',
            color: '#e74a3b',
            data: @json($data['chart_keluar'] ?? [], JSON_NUMERIC_CHECK)
        }, {
            name: 'Sisa/Margin',
            color: '#f6c23e',
            dashStyle: 'dot',
            data: @json($data['chart_sisa'] ?? [], JSON_NUMERIC_CHECK)
        }]
    });
    ...
    Highcharts.chart('chartPinjam', {
        ...
        series: [{
            name: 'Transaksi',
            color: '#4e73df',
            // Tambahkan parameter JSON_NUMERIC_CHECK di sini
            data: @json($data['chart_count_pinjam'] ?? [], JSON_NUMERIC_CHECK)
        }]
    });
</script>
@endsection