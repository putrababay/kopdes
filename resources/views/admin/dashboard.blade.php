...
<style>
    ...
    .battery-container { border: 3px solid #333; border-radius: 10px; padding: 4px; position: relative; width: 100%; height: 50px; background: #fff; box-shadow: inset 0 2px 5px rgba(0,0,0,0.1); }
    .battery-container::after { content: ''; position: absolute; right: -10px; top: 12px; width: 7px; height: 20px; background: #333; border-radius: 0 4px 4px 0; }
    .battery-level { height: 100%; border-radius: 6px; transition: width 1s ease-in-out; display: flex; align-items: center; justify-content: center; color: white; font-weight: bold; }
    .card-today { background: linear-gradient(135deg, #4e73df 0%, #224abe 100%); color: white; border: none; transform: scale(1.02); }
</style>
...
<script>
    ...
    Highcharts.chart('chartGrowth', {
        ...
        yAxis: { title: { text: 'Nominal' } },
        tooltip: { shared: true, valuePrefix: 'Rp' },
        credits: { enabled: false },
        series: [{
            name: 'Pemasukan', color: '#1cc88a', data: {!! json_encode($data['chart_masuk']) !!}
        }, {
            name: 'Pengeluaran', color: '#e74a3b', data: {!! json_encode($data['chart_keluar']) !!}
        }, {
            name: 'Sisa/Margin', color: '#f6c23e', dashStyle: 'dot', data: {!! json_encode($data['chart_sisa']) !!}
        }]
    });
    ...
    Highcharts.chart('chartPinjam', {
        ...
        credits: { enabled: false },
        series: [{
            name: 'Transaksi', color: '#4e73df', data: {!! json_encode($data['chart_count_pinjam']) !!}
        }]
    });
</script>
@endsection